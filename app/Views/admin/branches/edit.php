...
<div class="card p-4">
    <form action="<?= base_url('admin/branches/update/' . $branch['id']) ?>" method="POST">
        <?= csrf_field() ?>
        
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="name" class="form-label fw-bold small text-dark">Branch Name</label>
                <input type="text" name="name" id="name" class="form-control" value="<?= esc($branch['name']) ?>" required>
            </div>
            
            <div class="col-md-6 mb-3">
                <label for="phone" class="form-label fw-bold small text-dark">Contact Phone</label>
                <input type="text" name="phone" id="phone" class="form-control" value="<?= esc($branch['phone']) ?>" required>
            </div>

            <div class="col-md-6 mb-3">
                <label for="email" class="form-label fw-bold small text-dark">Email Address</label>
                <input type="email" name="email" id="email" class="form-control" value="<?= esc($branch['email']) ?>" required>
            </div>

            <div class="col-md-6 mb-3">
                <label for="map_url" class="form-label fw-bold small text-dark">Google Maps Location URL</label>
                <input type="url" name="map_url" id="map_url" class="form-control" value="<?= esc($branch['map_url']) ?>" required>
            </div>

            <div class="col-md-12 mb-4">
                <label for="address" class="form-label fw-bold small text-dark">Clinic Address</label>
                <textarea name="address" id="address" class="form-control" rows="4" required><?= esc($branch['address']) ?></textarea>
            </div>
        </div>
